...
namespace BrowserAutomationMaster
{
    ...
    internal static class CompilationHandler
    {
        ...
        public static (bool, string) Visit(
            List<string> scriptBody, List<string> featureLines, string sanitizedArg2, string selectedBrowser, bool firstVisitFinished, bool disableSSL, bool runHeadless)
        {
            if (!Transpiler.IsResolvableLink(sanitizedArg2))
            {
                Errors.WriteErrorAndExit(
            ...
            {
                scriptBody.AddRange(["print('Initializing WebDriver...')\n",
                                                 "driver = None",
                                                 "status_code = None",
                                                 "final_url = url",
                                                 "request_url = None"]);

                string proxyLine = featureLines.Where(x =>
                                                      x.Contains("use-") &&
                                                      x.Contains("-proxy")).FirstOrDefault("");
                ...
                switch (selectedBrowser)
                {
                    ...
                    case "firefox" or "safari":
                        ...
                        { // Disables SSL
                            ...
                            scriptBody.Add("try:");
                            scriptBody.Add($"{Transpiler.Indent(1)}driver = webdriver.Firefox(service=FirefoxService(GeckoDriverManager().install()), options=options, seleniumwire_options=sw_options)");
                            scriptBody.Add("except Exception as e:");
                            scriptBody.Add($"{Transpiler.Indent(1)}if 'cannot find Firefox binary' in str(e):\n");
                            scriptBody.Add($"{Transpiler.Indent(2)}stderr.write('Please install firefox and try running again.')");
                            scriptBody.Add($"{Transpiler.Indent(2)}exit(1)");
                        }
                        else
                        { // Uses SSL
                            scriptBody.Add("try:");
                            scriptBody.Add($"{Transpiler.Indent(1)}driver = webdriver.Firefox(service=FirefoxService(GeckoDriverManager().install()), seleniumwire_options=sw_options)");
                            scriptBody.Add("except Exception as e:");
                            scriptBody.Add($"{Transpiler.Indent(1)}if 'cannot find Firefox binary' in str(e):\n");
                            scriptBody.Add($"{Transpiler.Indent(2)}stderr.write('Please install firefox and try running again.')");
                            scriptBody.Add($"{Transpiler.Indent(2)}exit(1)");
